...
void MuscleLimitConstraint::update()
{
	...
	mViolation = (l_mt - mMuscle->GetLMTmax());
...
void MuscleLimitConstraint::getVelocityChange(double* _delVel, bool _withCfm)
{
...
void MuscleLimitConstraint::excite()
{
	mSkeleton->setImpulseApplied(true);
}

void MuscleLimitConstraint::unexcite()
{
	mSkeleton->setImpulseApplied(false);
}

void MuscleLimitConstraint::applyImpulse(double* _lambda)
{
...
MuscleLimitConstraint::getRootSkeleton() const
{
	return mSkeleton->mUnionRootSkeleton.lock();
}

bool MuscleLimitConstraint::isActive() const
{
	return mActive;
}
